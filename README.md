...
- 内置 dashboard panel：日历、feed、Hacker News、文章、GitHub、Agent runs、Markdown view。
- Agent Provider 抽象，首批支持 `local-command` 和 `codex-cli`。
- Task 抽象，首个内置任务可通过 Codex CLI 采集最近 24 小时 Hacker News Top 10 并累积为本地知识。
...
```bash
go run ./apps/daymine/cmd/daymine --addr :6345 --scheduler
```
