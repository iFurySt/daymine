...
- `packages/tasks/`：可调度任务目录，定义任务输入、provider、产物路径和 panel 关联。
...
3. 用户或 scheduler 触发 task，task registry 将模板输入交给对应 Agent Provider。
4. Agent controller 创建 run id，调用配置好的 provider。
5. Provider 在受控 workspace 中执行，产出消息、日志和文件。
6. Controller 记录 append-only run 事件，并刷新 panel 索引。
7. 前端通过 API 重新读取 panel 数据和 run 状态。
...
- Task 可调度：任务只声明输入、调度频率、provider 和产物契约；是否由 Codex、local command 或未来 agent 执行由 provider 决定。
