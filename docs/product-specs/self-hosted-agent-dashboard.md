...
Task 是 Provider 之上的稳定产品层，负责把「要做什么」和「由谁执行」解耦：

- 输入：任务 id、标题、描述、provider、prompt、调度频率、预期 artifact 和关联 panel。
- 执行：用户点击或 scheduler 触发 task，后端把 task prompt 交给对应 Agent Provider。
- 输出：沿用 Agent run JSONL 记录，并通过 workspace artifact 被 panel 读取。
- 首个内置 task：`hacker-news-daily-top10`，使用 `codex-cli` 获取最近 24 小时 Hacker News Top 10，写入 `index/hacker-news/` 和 `notes/sources/hacker-news/`。
...
- `hacker-news-top`：最近一次 Hacker News 每日 Top 10 digest。
